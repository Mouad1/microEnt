...
test.describe('Application Features with Bypassed Authentication', () => {
  ...
  test('dashboard displays actual content without auth redirect', async ({
    page,
  }) => {
  ...
  test('activity report form is functional', async ({ page }) => {
    ...
    if (formCount > 0) {
      ...
      // Test text inputs
      const textInputs = firstForm.locator(
      ...
      const buttons = firstForm.locator(
        'button[type="submit"], button:has-text("Generate"), button:has-text("Submit")'
      );
  ...
  test('navigation between authenticated pages works', async ({ page }) => {
    ...
    for (const testPage of pages) {
      ...
      await expect(page.locator(testPage.component)).toBeVisible({
        timeout: 10000,
      });
  ...
  test('ui components render correctly after authentication', async ({
    page,
  }) => {
    await page.goto('/dashboard');
    await page.waitForSelector('app-root', { timeout: 10000 });

...
test.describe('Post-Authentication E2E Tests', () => {
  ...
  test('dashboard loads and displays content after authentication', async ({ page }) => {
    ...
    const pageTitle = page.locator('h1, [data-testid="page-title"], .page-title');
    if (await pageTitle.count() > 0) {
      await expect(pageTitle.first()).toBeVisible();
    }
    
    // Check for dashboard widgets/cards
    const dashboardContent = page.locator('.dashboard, .stats, .widgets, [data-testid="dashboard-content"]');
  ...
  test('navigation between pages works after authentication', async ({ page }) => {
    // Start at dashboard
    await page.goto('/dashboard');
    await expect(page.locator('app-dashboard')).toBeVisible();
  ...
  test('user can interact with forms after authentication', async ({ page }) => {
    await page.goto('/activity-report');
    await expect(page.locator('app-activity-report')).toBeVisible();
  ...
  test('admin user can access admin features', async ({ page }) => {
    ...
    if (isConfiguration) {
      ...
      console.log('⚠️ Admin access to configuration blocked - check role configuration');
    } else {
    ...
    const isDocumentation = await page.locator('app-documentation-viewer').isVisible();
    if (isDocumentation) {
